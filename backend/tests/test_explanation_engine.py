...
def test_explanation_mentions_deficient_nutrients_in_plain_language():
    ...
    assert (
        "Nitrogen" in explanation.summary
        or "Phosphorus" in explanation.summary
    )
    ...
    # into farmer-facing explanations.
    joined = " ".join(
        explanation.identified_issues
    ).lower()
...
def test_explanation_for_healthy_soil_has_no_actions_needed_message():
    ...
    assert (
        "healthy" in explanation.summary.lower()
        or "no major" in explanation.summary.lower()
    )

    assert any(
        "no additional fertilizer" in action.lower()
